...
+(NSDictionary *)selectUserByName:(NSString *)username
{
    ...
    NSString *sqlQuery = [[NSString alloc]initWithFormat:@"SELECT USERID,USERNAME,PASSWORD, WEIGHT,BRITHDAY,GENDER,HEIGHT,PROFESSION FROM USER WHERE USERNAME = '%@' ORDER BY USERID ASC",username];
...
+(BOOL)initDataBase
{
    ...
    NSString *sqlCreateUserTable = @"CREATE TABLE IF NOT EXISTS USER(USERID INTEGER PRIMARY KEY AUTOINCREMENT, USERNAME TEXT, BIRTHDAY TIMESTAMP, AGE INTEGER, GENDER INTEGER, PROFESSION INTEGER, HEIGHT INTEGER, WEIGHT FLOAT, STEPLENGTH INTEGER, PASSWORD TEXT)";
    ...
    NSString *sqlCreateTemperatureTable = @"CREATE TABLE IF NOT EXISTS TEMPERATUREDATA(DATAID INTEGER PRIMARY KEY AUTOINCREMENT, USERID INTEGER, USERNAME TEXT, TEMPERTURE FLOAT, TESTTIME TIMESTAMP, DEVICEID TEXT, DEVICETYPE TEXT, ISSEND TEXT)";
